...
class JobsViewController: UIViewController {
    ...
    var editDelegate: EditJobDelegate!
...
extension JobsViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, FavoriteButtonDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: reuseIdentifier, for: indexPath) as! JobCollectionViewCell
    ...
    func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        ...
        if JobsViewController.animatedCells < jobsToAnimate {
            ...
            cell.frame = CGRect(x: cell.frame.origin.x-view.frame.width, y: cell.frame.origin.y, width: cell.frame.size.width, height: cell.frame.size.height)
...
extension JobsViewController: AddJobDelegate {
    ...
    func saveButtonTapped(company: Company) {
        for i in 0..<companies.count {
            if companies[i].dateAdded == company.dateAdded {
                companies[i] = company
