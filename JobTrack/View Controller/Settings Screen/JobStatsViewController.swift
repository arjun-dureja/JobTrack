...
class JobStatsViewController: UIViewController {
    ...
    func style() {
        ...
        pieChart.centerAttributedText = NSAttributedString(string: "Total: \(companies.count)", attributes: [NSAttributedString.Key.font: UIFont.boldSystemFont(ofSize: 16), NSAttributedString.Key.foregroundColor: UIColor.label, NSAttributedString.Key.paragraphStyle: paragraphStyle])
    ...
    func setupData() {
        ...
        dataSet.colors = [.offerBackground, .onSiteBackground, .phoneScreenBackground, .appliedBackground, .rejectedBackground]
...
extension JobStatsViewController: UIActivityItemSource {
    ...
    func activityViewControllerLinkMetadata(_ activityViewController: UIActivityViewController) -> LPLinkMetadata? {
        ...
        if chartToShare == 0 {
            let renderer = UIGraphicsImageRenderer(size: self.pieChart.bounds.size)
            chartImage = renderer.image { ctx in
                self.pieChart.drawHierarchy(in: self.pieChart.bounds, afterScreenUpdates: true)
            }
